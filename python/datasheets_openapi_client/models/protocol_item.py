...
class ProtocolItem(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProtocolItem:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ProtocolItem:
        """Create an instance of ProtocolItem from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ProtocolItem.parse_obj(obj)

        # raise errors for additional fields in the input
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ProtocolItem) in the input: " + obj)

        _obj = ProtocolItem.parse_obj({
            "name": obj.get("name")
        })
        return _obj
